...
var globalCorrelationTracker = NewCorrelationTracker()
...
// Reset clears all tracked windows (useful for testing or reload)
...
func (ct *CorrelationTracker) RecordAndCheck(ruleID string, groupKey string, eventTimeNano int64, conf *CorrelationConfig) bool {
	...
	if eventTimeNano-ct.lastCleanTime > int64(5*time.Minute) {
