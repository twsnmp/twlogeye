...
func checkUpdate() {
	...
	latest, ok, err := selfupdate.DetectLatest("twsnmp/twlogeye")
	if err != nil {
	...
	if latest.Version.GT(v) {
		...
		fmt.Println("Run 'twlogeye update' to upgrade.")
	} else {
		fmt.Printf("twlogeye is up to date (v%s).\n", v)
	}
}
...
func updateToVersion(target string, yes bool) {
	...
	rel, ok, err := selfupdate.DetectVersion("twsnmp/twlogeye", "v"+targetV.String())
	if err != nil {
