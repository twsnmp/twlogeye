...
func (m dashboardModel) renderMqttCount() string {
	height := 6
	width := (m.width / 2) - 2
	if m.width < 80 {
		width = m.width - 2
	...
	row := []string{}
	if len(m.mqttReport) > 0 {
		counts := []float64{}
		types := []float64{}
		for _, e := range m.mqttReport {
			counts = append(counts, float64(e.Count))
			types = append(types, float64(e.Types))
		}
		leftTitle := "MQTT Count " + time.Unix(0, m.mqttReport[len(m.mqttReport)-1].Time).Format("2006-01-02 15:04")
		...
		row = append(row, formatMqttCountLine("Count", counts, width, lipgloss.NewStyle().Foreground(ColorBlue)))
		row = append(row, formatMqttCountLine("Types", types, width, lipgloss.NewStyle().Foreground(ColorGray)))
	} else {
		row = append(row, titleStyle.Render("MQTT Count"))
		row = append(row, "No MQTT report")
	}
	return style.Render(lipgloss.JoinVertical(lipgloss.Left, row...))
}

func formatMqttCountLine(l string, values []float64, width int, style lipgloss.Style) string {
...
func (m dashboardModel) renderMqttTypes() string {
	height := 6
	width := (m.width / 2) - 2
	if m.width < 80 {
		width = m.width - 2
	...
	row := []string{}
	if len(m.mqttReport) > 0 {
		last := m.mqttReport[len(m.mqttReport)-1]
		row = append(row, titleStyle.Render(fmt.Sprintf("MQTT Patterns %d patterns from %d logs",
			last.Types, last.Count)))
		row = append(row, m.renderMqttTypesContent(width))
	} else {
		row = append(row, titleStyle.Render("MQTT Patterns"))
		row = append(row, "No mqtt report")
	}
	return style.Render(lipgloss.JoinVertical(lipgloss.Left, row...))
}

func (m dashboardModel) renderMqttTypesContent(chartWidth int) string {
	types := m.mqttReport[len(m.mqttReport)-1].TopList
	maxCount := 0
	for _, t := range types {
	...
	for i := 0; i < topNLines; i++ {
		if i < len(types) {
			...
			mqttType := fmt.Sprintf("%s %s", t.ClientId, t.Topic)
			if len(mqttType) > mqttTypeWidth {
				mqttType = mqttType[:mqttTypeWidth-3] + "..."
			}
			...
			line := fmt.Sprintf("%s %s │ %s",
				...
				lipgloss.NewStyle().Foreground(ColorWhite).Render(mqttType),
			)
